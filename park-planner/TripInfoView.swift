...
struct TripInfoView: View {
    ...
    func checklistProgress(for trip: Trip) -> Double {
        guard !trip.checklist.isEmpty else { return 0 }
        let completed = trip.checklist.filter { $0.isCompleted }.count
        return Double(completed) / Double(trip.checklist.count)
    }
    ...
    @ViewBuilder
    func tripCard(for index: Int) -> some View {
        let trip = trips[index]
        let isSelected = selectedTrip == trip.id
        let days = daysUntil(trip.startDate)
        let progress = checklistProgress(for: trip)
        let completedCount = trip.checklist.filter { $0.isCompleted }.count

        VStack(alignment: .leading, spacing: 14) {

            // Top row: name + countdown badge
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: 4) {
                    if isSelected {
                        Text("SELECTED")
